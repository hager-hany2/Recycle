...
class ChatbotConfig:
    def __init__(self, data_path='englishverison.json', model_path='model.keras'):
        # Load intents data
        ...
        for example in self.data['data']:
            for pattern in example['patterns']:
                ...
                dic['label'].append(example['label'])
    ...
    def predict_response(self, user_input):
        ...
        responses = self.df[self.df['label'] == tag]['responses'].values[0]
